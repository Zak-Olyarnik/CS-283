...
void eval(char *cmdline) 
{
	...
	if (!builtin_cmd(argv))		// built-in commands handle themselves
	{
		/*
		...
		{
			sigprocmask(SIG_UNBLOCK, &mask, NULL);	// child should unblock
		...
		{
			addjob(jobs, pid, FG, buf);	// add job before unblocking for safe control flow
			sigprocmask(SIG_UNBLOCK, &mask, NULL);	// unblock signals 
			waitfg(pid);	// wait until process terminates or is sent to bg
		}
		else		/* otherwise, don't wait for bg job */
		{
			addjob(jobs, pid, BG, buf);
			// make output like test files
			printf("[%d] (%d) %s", getjobpid(jobs, pid)->jid, pid, buf);
		}
	}
	return;
}
...
void do_bgfg(char **argv) 
{
	if(argv[1] == NULL)	// no pid or jid provided
	{
		// make output like test files
	...
	if(argv[1][0] == '%')	// check for jid delimiter (argv[0] is
	{									// fg or bg, argv[1] is jid or pid)
		j = 1;
		jid = 1;
	}
	else
	...
	for (i=j; i< strlen(argv[1]); i++)	// check jid/pid is numeric
	{
...
void waitfg(pid_t pid)
{
	/* Wait for process pid to stop being the foreground process.
		Find the process in the job list, then
		check its state, goto sleep, repeat
	 */
...
void sigchld_handler(int sig) 
{
	...
	{	// pid will be 0 if no children, PID of terminated child otherwise
		struct job_t *job = getjobpid(jobs, pid);
		...
		if(WIFSTOPPED(status))
		{
			...
			// WTERMSIG returns the number of the signal that caused the stop
		}
...
void sigint_handler(int sig) 
{
	...
	if(pid > 0)
	{
		...
		kill(-pid, SIGINT);	// "-" sends signal to entire process group
	}
   return;
}
...
void sigtstp_handler(int sig) 
{
	...
	if(pid > 0)
	{
		...
		kill(-pid, SIGTSTP);	// "-" sends signal to entire process group
	}
	return;
}
